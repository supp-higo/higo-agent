...
deploy:
	...
	(uv export --no-hashes --no-header --no-dev --no-emit-project --no-annotate > agents/shared/.requirements.txt 2>/dev/null || \
	uv export --no-hashes --no-header --no-dev --no-emit-project > agents/shared/.requirements.txt) && \
	uv run -m agents.shared.deploy \
		--source-packages=./agents \
		--entrypoint-module=agents.discovery.agent_engine_app \
		--entrypoint-object=agent_engine \
		--requirements-file=agents/shared/.requirements.txt \
